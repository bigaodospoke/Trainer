...
export function Topbar({ name, avatarUrl, onMenuClick }: TopbarProps) {
  return (
    <header className="flex h-16 shrink-0 items-center gap-3 border-b border-white/5 bg-void/70 px-4 backdrop-blur-xl sm:gap-4 sm:px-6">
      <button
        type="button"
        onClick={onMenuClick}
        aria-label="Abrir menu"
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-ink-muted transition-colors hover:bg-white/5 hover:text-ink-primary lg:hidden"
      >
      ...
      <div className="relative min-w-0 flex-1 sm:max-w-md">
        ...
        <input
          type="search"
          placeholder="Buscar Pokemon, times, usuarios..."
